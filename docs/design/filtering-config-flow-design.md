# Skolmat filtering config flow design (v2.1+)

This document captures the design decisions for the Home Assistant config/options flow
that sets up menu summary filtering. It is intended to sit alongside
`docs/filtering-design-checkpoint.md` and should be kept consistent with it.

Goal: a guided flow that stays honest, avoids surprise filtering, and still lets power
users tune the summary output for noisy providers.

Conflict rule: If this doc conflicts with `docs/filtering-design-checkpoint.md`,
the checkpoint wins unless explicitly superseded here.
...
- Exclusions:
  - Label selection from discovered labels (primary UI).
  - Regex patterns (advanced toggle).
  - Target scope: label, dish, or both (default label-first).
- Preferences:
  - Label selection from discovered labels.
  - Regex patterns (advanced toggle).
  - Target scope: label, dish, or both.
...
Options should mirror config flow but start at "Mode + Filters".
...
### 3.1 Mode selection copy
- Auto: "Best-effort summary, minimal setup. No default limits."
- Custom: "Pick which meals are eligible. Advanced filters optional."

### 3.2 Warnings and transparency
...
Filters:
- `exclude_labels`: list of strings (exact match)
- `exclude_regex`: list of patterns (advanced)
- `exclude_target`: `label` | `dish` | `both`
- `prefer_labels`: list of strings (exact match)
- `prefer_regex`: list of patterns (advanced)
- `prefer_target`: `label` | `dish` | `both`
